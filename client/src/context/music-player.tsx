...
export const PlayerContextProvider: React.FC<PlayerProviderProps> = ({ children }) => {
  ...
  const nextTrack = useCallback(() => {
    if (currentTrack && albumData.length > 0) {
      ...
      const nextIndex = (currentIndex + 1) % albumData.length;
  ...
  const previousTrack = useCallback(() => {
    if (currentTrack && albumData.length > 0) {
      const currentIndex = albumData.findIndex((song) => song.id === currentTrack.id);
      const prevIndex = (currentIndex - 1 + albumData.length) % albumData.length;
  ...
  const playWithId = useCallback(
    (songId: string | number) => {
      const song = albumData.find((s) => s.id === Number(songId));
      if (song) {
